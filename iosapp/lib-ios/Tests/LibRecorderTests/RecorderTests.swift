...
final class RecorderTests: XCTestCase {
  ...
  func testTimingWithWrongSystemClock() {
    ...
    XCTAssertFalse(self._isRecording(lastRecordedSampleTime: nil, currentTime: now))

    XCTAssertTrue(self._isRecording(lastRecordedSampleTime: oneSecondAgo, currentTime: now))
    XCTAssertTrue(self._isRecording(lastRecordedSampleTime: fourSecondsAgo, currentTime: now))
    XCTAssertFalse(self._isRecording(lastRecordedSampleTime: tenSecondsAgo, currentTime: now))

    XCTAssertFalse(self._isRecording(lastRecordedSampleTime: oneSecondFuture, currentTime: now))
    XCTAssertFalse(self._isRecording(lastRecordedSampleTime: fourSecondsFuture, currentTime: now))
    XCTAssertFalse(self._isRecording(lastRecordedSampleTime: tenSecondsFuture, currentTime: now))
  }

  // Same logic as RecordingStatus
  func _isRecording(lastRecordedSampleTime: Date?, currentTime: Date) -> Bool {
    guard let lastRecordedSampleTime else { return false }
