...
final class ControllerProxyTests: XCTestCase {
  // TODO: decide if to try to salvage these tests
  // there is no heartbeat anymore, but we can trigger a refresh by calling decide flow
  // the controller proxy obviously needs lots of tests, but when...
  ...
  //       return [.bundleIdContains(value: "bad"), .bundleIdContains(value: "bad2")]
  //     }
