...
final class AuthedAdminResolverTests: ApiTestCase, @unchecked Sendable {
  ...
  func testGetAdminKeychains() async throws {
    ...
    let littleJimmy = try await self.db.create(
      User(
        id: .from("00000000-0000-0000-0000-000000000000"), parentId: admin.model.id,
        name: "Little Jimmy"
      )
    )
    ...
    let sally = try await self.db.create(
      User(
        id: .from("11111111-1111-1111-1111-111111111111"), parentId: admin.model.id,
        name: "Sally"
      )
    )

    let fromResolver = try await GetAdminKeychains.resolve(in: context(admin))
